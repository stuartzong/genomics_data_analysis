...
python expands.py -dt wgs or spc > summary_log_file.txt

"""
def make_patient_files_dict_new(bam_vcf_files):
    """
    dictionary holds all files: patient -> status -> file_identifier -> file_path_name
    """
    patient_files = dict()
    with open(bam_vcf_files, 'r') as fh:
        records = csv.DictReader(fh,  delimiter='\t')
        for line in records:
            patient = line['Line_name']
            status = line['status']
            lib = line['DNA_lib']
            ...
            #value = [lib, single_vcf, paired_vcf, strelka_snv_vcf, bam, tc, cnv, strelka_indel_vcf]
            tmp_dict = {'lib':lib,
...
def make_patient_files_dict(bam_vcf_files):
    ...
    with open(bam_vcf_files, 'r') as fh:
        ...
        for line in records:
            ...
            copy_cn_file(patient, cn_file)
            try:
                patient_files[patient].append(value)
            except KeyError:
                    patient_files[patient] = value   
        return [patient_files, library_status]
...
def summarize_snvs_wn(patient_files, sum_header_wn_tmp, hm_sum_wn_tmp, low_sum_wn_tmp):
    """ 
...
def parse_mpileup_indel(vcf_file, caller, impacts, gene_pats, gene_variants, patient_status):                                                          
    ...
    with open(vcf_file,  'r') as fh:
        ## Following line is useful for testing the script
        #fh = [next(fh) for x in xrange(100)]
        for line in fh:
            if (not line.startswith("#")):
                sl = line.strip().split("\t")
                Info = sl[7]
        ...
        for line in fh:
            if (not line.startswith("#")):
                ...
                if ("EFF=" in Info) and any(x in Info for x in impacts):
                            ...
                            cov_info = "\t".join(mpileup + strelka_cov)
...
def adjust_allele_frequency(com_af_files,patient_files):
    for file in com_af_files:
        ...
        print "Finishing af adjustment for: %s" % (patient)
...
def make_patient_vcfs_dict_to_fix(patient_files):
    # generate dict to hold normal and no-normal vcf files
    patient_wn_snv_vcfs = dict()
    patient_wn_indel_vcfs = dict()
    patient_non_vcfs = dict()
    for patient in patient_files:
        if ("normal" in patient_files[patient]):
            ...
            for status in patient_files[patient]:
                single_vcf = patient_files[patient][status][1]
                try:
                    patient_non_vcfs[patient].append(single_vcf)
                except KeyError:
                    patient_non_vcfs[patient] = [single_vcf]

    ##print intersect_scripts
    #print complete_stamp
    return [patient_wn_snv_vcfs, patient_wn_indel_vcfs, patient_non_vcfs]
    ...
    for patient in patient_files:
        ...
        for status in patient_files[patient]:
            for type in data_types:
                ...
                print "For %s %s tumor, example snvs are:\n" % (patient, status)
        ...
        if ("normal" in patient_files[patient]):
            ...
            print "%s has no normal and merging is not required!\n" % patient
...
def parse_pileup_output_to_delete(patient_files, patient_snv_wn, patient_snv_non):
    af_files = []
    com_af_files = []
    af_dicts = dict()
    data_types = ["DNA", "RNA"]
    for patient in patient_files:
        ...
        for status in patient_files[patient]:
            for data_type in data_types:            
                base_count_file  = ".".join([patient, status, data_type, "pileup.AFcounts"])
                af_file = ".".join([patient, status, data_type, "pileup.AFcounts.af"])
                af_files.append( af_file )

                # variant bp count dict for primary, relapse, and normal
                if ("DNA" in data_type):
                    DNA_af_dict  = count_ref_alt_bases(snp_list, base_count_file, af_file)
                if ("RNA" in data_type):
                    RNA_af_dict  = count_ref_alt_bases(snp_list, base_count_file, af_file)
                
                # '5:131892979:G': ['G:88:1:1:0.01']
                print "For %s %s %s tumor, example snvs are:\n" % (patient, status, data_type)
                for x in range(5):
                    print snp_list[x]
  
            # af_dicts hold both  DNA_af_dict and RNA_af_dict for each patient_status
            try:
                #print patient, status
                af_dicts[patient][status] = [DNA_af_dict, RNA_af_dict]
            except KeyError:
                #print "key error!"
                if patient not in af_dicts:
                    af_dicts[patient] = {}
                if status not in af_dicts[patient]:
                    af_dicts[patient][status] = [DNA_af_dict, RNA_af_dict]

        print "Merging af files for tumour/normal pairs!"
        if ("normal" in patient_files[patient]):
            n_af_dict_list = af_dicts[patient]["normal"]
            for status in patient_files[patient]:
                if (not status == "normal"):
                    com_af_file = ".".join([patient, status, "af.combined"])
                    com_af_files.append(com_af_file)
                    t_af_dict_list = af_dicts[patient][status]

                    merge_normal_tumor_afs(snp_list, n_af_dict, t_af_dict, com_af_file)
                    ...
                    with open(com_af_file,  'wb') as opf:
                        ...
                        for snp in snp_list:
                           ...
                           #normal DNA counts
                           try:
                               normal_afs = n_af_dict_list[0][af_key]
                               normal_af = [i for i in normal_afs if i[0] == alt][0].split(":")
                               #print "normal_af is: %s, %s " % (type(normal_af), normal_af)
                           except KeyError:
                               normal_af = ["N", "0", "0", "0", "0"]
                           ...
                           # normal RNA counts
                           try:
                               RNA_normal_afs = n_af_dict_list[1][af_key]
                               RNA_normal_af = [i for i in RNA_normal_afs if i[0] == alt][0].split(":")
                               #print "normal_af is: %s, %s " % (type(normal_af), normal_af)
                           except KeyError:
                               RNA_normal_af = ["N", "0", "0", "0", "0"]
                               #print "normal KEYERROR! %s \n" % normal_af
                           RNA_n_totC, RNA_n_refC, RNA_n_altC, RNA_n_af = RNA_normal_af[1], RNA_normal_af[2], RNA_normal_af[3], RNA_normal_af[4]
                           # tumor DNA counts
                           try:
                               tumour_afs = t_af_dict_list[0][af_key]
                               tumour_af = [i for i in tumour_afs if i[0] == alt][0].split(":")
                           except KeyError:
                               tumour_af = ["N", "0", "0", "0", "0"]
                               #print "tumour KEYERROR! %s\n" % tumour_af
                           ...
                           # tumor RNA counts
                           try:
                                RNA_tumour_afs = t_af_dict_list[1][af_key]
                                RNA_tumour_af = [i for i in  RNA_tumour_afs if i[0] == alt][0].split(":")
                           except KeyError:
                                RNA_tumour_af = ["N", "0", "0", "0", "0"]
                               #print "tumour KEYERROR! %s\n" % tumour_af
                           RNA_t_totC,  RNA_t_refC,  RNA_t_altC,  RNA_t_af =  RNA_tumour_af[1],  RNA_tumour_af[2],  RNA_tumour_af[3],  RNA_tumour_af[4]
...
def make_pileup_scripts(patient_files):
    ...
    for patient in patient_files:
        for status in patient_files[patient]:
            ...
            data["RNA"] = RNA_bam
            for datum in data: 
                script = make_pileup_script(patient, status, datum, snv_pos_file, data[datum])
                pileup_scripts.append( script )
                pileup_complete_stamp = "_".join([patient, status, datum, "pileup.complete"])
        ...
        with open(snv_sum,  'wb') as csvfile:
            ...
            with open(snv_sum_tmp,  'r') as snv_fh:
                 ...
                 for line in records:
                     ...
                     # af info 
                     try:
                       af = af_dict[variant][0].split(',')
                     except KeyError:
                        #print "eeeeeeeeeee", variant
                     ...
                     final_content = line_content + af + [tc]
...
def summarize_snvs_for_clonality(patients, vcfs, sum_header_wn_tmp, hm_sum_wn_tmp, low_sum_wn_tmp, modifier_sum_wn_tmp):
    """ 
    Read in vcfs from both mpileup and strelka, 
    Generate triple nested dictionary for each
    Combine all the SNV positions from each pileline and patient status for pileup 
    Positions = mpileup + strelka + primary + relaspe
    Get snp list for all patients
    ...
    hm_impacts = ["HIGH", "MODERATE"]
    low_impacts = ["LOW"]
    modifier_impacts = ["MODIFIER"]
    snv_pos_file = "vcf.snp.pos"
    snv_pos = []
    snv_list = []
    caller = "unknown"
    # get pooled variant positions
    for vcf in vcfs:
        with open(vcf,  'r') as fh:
            ## Following line is useful for testing the script
            #fh = [next(fh) for x in xrange(2000)]
            print "Processing: ", vcf
            for line in fh:
                if (not line.startswith("#")):
                    sl = line.strip().split("\t")
                    Info = sl[7]
                    if (not Info.startswith("INDEL")) and ("EFF=" in Info):
                        splitInfo = Info.split(";")
                        # get gmaf value      
                        try:
                           gmaf = [j for j in splitInfo if
                        ...
                        for alt in alts:
                            ...
                            try:
                               ...
                               if snp_id_tmp.startswith("COS"):
                                   ...
                                   cosmic_id = "not_in_cosmic64"
                            if any(x in Info for x in hm_impacts) and (snv not in snv_list):
                                snv_pos.append("\t".join([chr, start]))
                                snv_list.append(snv)
                                for patient in patients:
                                    two_dicts = hold_snv_info(transcripts, hm_impacts, patient, snv, snp_id, cosmic_id, gmaf, caller,  hm_gene_dict, hm_variants_dict)
                                    hm_gene_dict = two_dicts[0]
                                    hm_variants_dict = two_dicts[1]
    snv_pos = list(set(snv_pos))
    snv_list = list(set(snv_list))
    ...
    n_items = take(2, patient_snv.iteritems())
    print n_items
    write_summary(hm_variants_dict, hm_gene_dict, hm_sum_wn_tmp, sum_header_wn_tmp)
    print snv_list, snv_pos_file
    return [snv_list, snv_pos_file]
...
def __main__():
    ...
    print "This pipeline does the following:\n \
           -> Combine variants from samtools mpileup and strelka pipeline etc,\n \
           ...
           -> Combining base count info for tumour-normal pairs, \n \
    ...
    parser = argparse.ArgumentParser(description='Summarize variants at gene andvariant level')
    parser.add_argument('-dt','--data_type', help='specify data type as wgs_with_normal or wgs_wo_normal', required=True)
    args = vars(parser.parse_args())

    input_files = "/projects/trans_scratch/validations/workspace/szong/David_Kaplan/variants/run2/NB_TIC_lines_for_BC_genome_normal_cn_unknown.csv" 
    ...
    cosmic_anno = "/home/szong/projects/resource/cosmic/" \
                           "CosmicMutantExport.v71.tsv"
    ...
    sum_snv_header_non = sum_snv_header + ["tumourDNASequencingCoverage", "tumourDNARefBaseCount", 
                             ...
                             "tumourContent"]
    sum_snv_header_wn = sum_snv_header + ["normalSequencingCoverage", "normalRefBaseCount", 
    ...
    patient_files = make_patient_files_dict_new(input_files)
    ...
    if args['data_type'] == 'wgs':
        ...
        sum_out_wn = summarize_snvs_wn(patient_files_wn, sum_snv_header, hm_snv_sum_wn_tmp, low_snv_sum_wn_tmp)
        ...
        sum_out_non = summarize_snvs_wn(patient_files_non, sum_snv_header, hm_snv_sum_non_tmp, low_snv_sum_non_tmp)
        ...
        combine_sum_af_anno(pairing_status, hm_snv_sum_wn, hm_snv_sum_wn_tmp, 
                            af_wn_dict, patient_files, sum_snv_header_wn)
