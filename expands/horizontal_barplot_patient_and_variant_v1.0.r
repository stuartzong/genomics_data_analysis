...
par(mar= c(5,4,4,2))
pdf(file.path(".", "patient_variant.pdf"), height=10, width=8)
...
file <-"genome_SNVs_histogram.txt"
...
num_pat <- data$numPatientGeneLevelFiltered
num_var <- data$numVariantsForThisGeneFiltered
str(num_pat)
...
mp <- barplot(height, axes=TRUE,col=cols,border=NA,cex.axis=1.2,cex.lab=1.3,xlim=c(0, 6),ylim=c(0,ylim),ylab="", xlab="Occurrence",xaxs="i",yaxs="i",horiz=TRUE, names.arg=gene.names, cex.names=0.9,las=1, beside=TRUE)
...
legend("topright", legend = c("patients", "uniq variants"), cex=1.3, fill = cols, bty="n")
