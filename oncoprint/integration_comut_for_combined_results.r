...
#library("ggplot2")
.libPaths('/gsc/software/linux-x86_64-centos6/R-3.3.2/lib64/R/library')
library(ggplot2)
library(plyr)
library(reshape2)
library(RColorBrewer)
#not sure why genom-points plotted for size=0

#this time just plot combined results
# RNA
...
# file <- "/projects/trans_scratch/validations/workspace/szong/Cervical/integration/82_patients/transcriptome/transcriptome_integration_summary.csv.added.formated.comut"
file <- "/projects/trans_scratch/validations/workspace/szong/Cervical/integration/101_patients/combined_integration_results.csv"
...
#DNA_file <- "/projects/trans_scratch/validations/workspace/szong/Cervical/integration/82_patients/genome/genome_integration_summary.csv.added.formated.comut"
df <- read.table(file,
                 header=TRUE,
                 sep="\t",
                 quote="",
...
# this is to manually order the legend, also to assign the right color to the categories, 
#look what is x, then match up it with the color while level it, need to understand better
x <- factor(df$status)
#x = factor(x,levels(x)[c(1,5, 4,3, 2,6)])
x = factor(x,levels(x)[c(1,6, 5,4, 3,2,7)])
...
#geom_point(data=df2, aes(x=patient, y=ids ), size=df2$status)+
xlab("") +
...
ggsave(mut,file="/projects/trans_scratch/validations/workspace/szong/Cervical/integration/101_patients/integration_Comutplot.png",width=14,height=8)
