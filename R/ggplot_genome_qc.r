...
my.path <- getwd()
files <- list.files(my.path, pattern = "bamstats_summary_transposed.csv$")
files
...
df2.molten <- melt(df2, value.name="qc_matrix", variable.name="library", na.rm=TRUE)
df2.molten.sorted <- arrange(df2.molten, library, Category) 

print("long format of the data")
print(df2.molten.sorted)
mut <- ggplot(data=df2.molten.sorted, aes(x=library, y=qc_matrix, group = Category, colour = Category)) +
    geom_line() +
    ...
    ## theme(axis.title.x = element_text(size = rel(3.8), angle = 90))
