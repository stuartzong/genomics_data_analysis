...
my.path <- getwd()
files <- list.files(my.path, pattern = "normoxic_NB333R_vs_L_somatic_SNVs.csv.box$")
files
...
print(
  ...
  geom_text(data=subset(df_cumsum, num_genes>100000 ), aes(y=label_ypos, label=num_genes), vjust=0.6, color="black", size=3.5)+
  #geom_text(data=subset(df_cumsum, num_genes=50), aes(y=label_ypos, label='genes'), vjust=0.6, color="black", size=3.5)+
